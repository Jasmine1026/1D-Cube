...
CBrake::CBrake() {
	...
	if (ret < 0) {
		...
		if (ret < 0) {
			cerr << "(CBrake::initPWM) sysfsEcho to export failed! ret: " << ret
					<< endl;
		}
	}
	...
	if (ret < 0) {
		cerr << "(CBrake::initPWM) sysfsEcho to " << enable_file
				<< " failed! ret: " << ret << endl;
	}
	...
	if (ret < 0) {
		cerr << "(CBrake::initPWM) sysfsEcho to " << polarity_file
				<< " failed! ret: " << ret << endl;
	}
	...
	if (ret < 0) {
		cerr << "(CBrake::initPWM) sysfsEcho to " << frequency_file
				<< " failed! ret: " << ret << endl;
	}
	...
	if (mServoPWMFd < 0) {
		cerr << "(CBrake::initPWM) open " << dc_file << " failed! ret: " << ret
				<< endl;
	}

	ret = write(mServoPWMFd, OPEN_DUTY_CYCLE, strlen(OPEN_DUTY_CYCLE));
	if ((ret < 0) || (ret != strlen("0"))) {
		cerr << "(CBrake::initPWM) write to " << dc_file << " failed! ret: "
				<< ret << endl;
	}
...
bool CBrake::openBrake() {
	...
	if (ret != static_cast<int>(strlen(OPEN_DUTY_CYCLE))) {
		cerr << "(CBrake::openBrake) write() to Servo-PWM-FD failed! ret : "
				<< ret << endl;
		return false;
	}
	return true;
...
bool CBrake::closeBrake() {
	...
	if (ret != static_cast<int>(strlen(CLOSE_DUTY_CYCLE))) {
		cerr << "(CBrake::closeBrake) write() to Servo-PWM-FD failed! ret: "
				<< ret << endl;
		return false;
	}
	return true;
