...
CMutex::CMutex(bool isFull, bool isProcessShared) {
	...
	sAssertion(0 == retVal,
			"(CMutex::CMutex()): Failed to initialize attributes", true);
	pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_RECURSIVE);
	retVal = pthread_mutexattr_setpshared(&attr,
			isProcessShared ? PTHREAD_PROCESS_SHARED : PTHREAD_PROCESS_PRIVATE);
	sAssertion(0 == retVal, "(CMutex::CMutex()): Failed to set attributes",
			true);

	retVal = pthread_mutex_init(&mMutex, &attr);
	sAssertion(0 == retVal,
			"(CMutex::CMutex()): Failed to initialize the mutex", true);
...
void CMutex::give() {
	...
	sAssertion(0 == retVal, "(CMutex::give()): Failed to unlock the mutex",
			true);
}
CMutex::~CMutex() {
