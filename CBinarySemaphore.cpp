...
CBinarySemaphore::CBinarySemaphore(bool isFull, bool isProcessShared) :
		mCounter(1) {
	Int32 retVal;
	pthread_mutexattr_t mutexAttr;
	retVal = pthread_mutexattr_init(&mutexAttr);
	sAssertion(0 == retVal,
			"(CBinarySemaphore::CBinarySemaphore()) : Failed to init Mutex-Attribute!",
			true);

	retVal = pthread_mutexattr_settype(&mutexAttr, PTHREAD_MUTEX_ERRORCHECK);
	sAssertion(0 == retVal,
			"(CBinarySemaphore::CBinarySemaphore()) : Failed to set Mutex-Type!",
			true);

	retVal = pthread_mutex_init(&mMutex, &mutexAttr);
	sAssertion(0 == retVal,
			"(CBinarySemaphore::CBinarySemaphore()) : Failed to init Mutex!",
			true);

	retVal = pthread_mutexattr_destroy(&mutexAttr);
	sAssertion(0 == retVal,
			"(CBinarySemaphore::CBinarySemaphore()) : Failed to destroy Mutex-Attribute!",
			true);

	pthread_condattr_t conditionAttr;
	retVal = pthread_condattr_init(&conditionAttr);
	sAssertion(0 == retVal,
			"(CBinarySemaphore::CBinarySemaphore()) : Failed to init Condition-Attribute!",
			true);

	retVal = pthread_condattr_setpshared(&conditionAttr,
			isProcessShared ? PTHREAD_PROCESS_SHARED : PTHREAD_PROCESS_PRIVATE);
	sAssertion(0 == retVal,
			"(CBinarySemaphore::CBinarySemaphore()) : Failed to set Condition-Attribute!",
			true);
