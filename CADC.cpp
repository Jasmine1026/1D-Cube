...
CADC::CADC() :
	...
	if (ret < 0) {
		cerr << "(CADC::CADC) sysfsEcho to " << enable_file << " failed! ret: "
				<< ret << endl;
		exit(-1);
	}
	...
	if (ret < 0) {
		cerr << "(CADC::CADC) sysfsEcho to " << channel_file << " failed! ret: "
				<< ret << endl;
		exit(-1);
	}
	...
	if (mADCFd < 0) {
		cerr << "(CADC::CADC) open() /dev/iio:device0 failed! ret: " << ret
				<< endl;
		exit(-1);
	}
}
...
bool CADC::fetchValue(UInt16& value) {
	...
	if ((ret < 0) || (ret < static_cast<int>(sizeof(UInt16)))) {
		cerr << "(CADC::fetchValues) read() mADCFd failed! ret: " << ret
				<< endl;
		return false;
	}
	return true;
