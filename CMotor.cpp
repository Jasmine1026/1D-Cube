...
CMotor::CMotor() :
		mDirectionPinFd(-1), mEnablePinFd(-1), mTorquePWMFd(-1) {
	//Initialize the direction and enable pins
	mEnablePinFd = this->initGPIOPin(66);
	if (mEnablePinFd < 0) {
		cerr << "(CMotor::CMotor) Failed to initialize enable pin! ret: "
				<< mEnablePinFd << endl;
		exit(-1);
	}
	mDirectionPinFd = this->initGPIOPin(67);
	if (mDirectionPinFd < 0) {
		cerr << "(CMotor::CMotor) Failed to initialize direction pin! ret: "
				<< mDirectionPinFd << endl;
		exit(-1);
	}
	//Configure the pwm output for the torque signal
	mTorquePWMFd = this->initPWM();
	if (mTorquePWMFd < 0) {
		cerr << "(CMotor::CMotor) Failed to initialize torque pwm! ret: "
				<< mTorquePWMFd << endl;
		exit(-1);
	}
}
...
bool CMotor::enableMotor() {
	...
	if (ret != strlen("1")) {
		cerr
				<< "(CMotor::enableMotor) Failed to write() to Enable-Pin-FD! ret: "
				<< ret << endl;
		return false;
	}
	return true;
...
bool CMotor::disableMotor() {
	...
	if (ret != strlen("0")) {
		cerr
				<< "(CMotor::disableMotor) Failed to write() to Enable-Pin-FD! ret: "
				<< ret << endl;
		return false;
	}
	return true;
...
bool CMotor::setTorque(float torque) {
	...
	int ret = write(mDirectionPinFd, directionPinValue,
			strlen(directionPinValue));
	if (ret != static_cast<int>(strlen(directionPinValue))) {
		cerr << "(CMotor::setTorque) write() to Direction-Pin-FD failed! ret: "
				<< ret << endl;
		return false;
	}
	...
	if (ret != static_cast<int>(strlen(dcCStr))) {
		cerr << "(CMotor::setTorque) write() to Torque-PWM-FD failed! ret: "
				<< ret << endl;
		return false;
	}
	return true;
...
int CMotor::initPWM() {
	...
	if (ret < 0) {
		...
		if (ret < 0) {
			cerr << "(CMotor::initPWM) sysfsEcho to export failed! ret: " << ret
					<< endl;
			return -1;
		}
	}
	...
	if (ret < 0) {
		cerr << "(CMotor::initPWM) sysfsEcho to " << enable_file
				<< " failed! ret: " << ret << endl;
		return -2;
	}
	...
	if (ret < 0) {
		cerr << "(CMotor::initPWM) sysfsEcho to " << polarity_file
				<< " failed! ret: " << ret << endl;
		return -3;
	}
	...
	if (ret < 0) {
		cerr << "(CMotor::initPWM) sysfsEcho to " << frequency_file
				<< " failed! ret: " << ret << endl;
		return -4;
	}
	...
	if (dutyCycleFd < 0) {
		cerr << "(CMotor::initPWM) open " << dc_file << " failed! ret: " << ret
				<< endl;
		return -5;
	}
	...
	if ((ret < 0) || (ret != strlen("0"))) {
		cerr << "(CMotor::initPWM) write to " << dc_file << " failed! ret: "
				<< ret << endl;
		return -6;
	}
...
int CMotor::initGPIOPin(UInt8 gpioNr) {
	...
	if (ret < 0) {
		...
		if (ret < 0) {
			cerr << "(CMotor::initGPIOPin) sysfsEcho to " << valueFile
					<< " failed! ret: " << ret << endl;
			return -1;
		}
	}
	...
	if (ret < 0) {
		cerr << "(CMotor::initGPIOPin) sysfsEcho to " << active_low_file
				<< " failed! ret: " << ret << endl;
		return -2;
	}
	...
	if (ret < 0) {
		cerr << "(CMotor::initGPIOPin) sysfsEcho to " << direction_file
				<< " failed! ret: " << ret << endl;
		return -3;
	}
	...
	if (pinFD < 0) {
		cerr << "(CMotor::initGPIOPin) open() " << valueFile << " failed! ret: "
				<< ret << endl;
		return -4;
	}
	...
	if (ret < 0) {
		cerr << "(CMotor::initGPIOPin) write() to " << valueFile
				<< " failed! ret: " << ret << endl;
		return -5;
	}
