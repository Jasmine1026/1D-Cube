...
CMPU6050::CMPU6050(UInt8 deviceNr) :
		mAccelXFd(-1), mAccelYFd(-1), mAccelZFd(-1), mAnglVelXFd(-1), mAnglVelYFd(
				-1), mAnglVelZFd(-1) {
	int ret = -1;
	...
	if (ret < 0) {
		cerr << "(CMPU6050::CMPU6050) access() " << sampling_frequency
				<< " failed! ret: " << ret << endl;
	}
	...
	if (ret < 0) {
		cerr << "(CMPU6050::CMPU6050) Echo to " << enable << "failed! ret: "
				<< ret << endl;
		exit(-1);
	}
	...
	if (ret < 0) {
		cerr << "(CMPU6050::CMPU6050) Echo to " << sampling_frequency
				<< " failed! ret: " << ret << endl;
		exit(-1);
	}
	...
	if (ret < 0) {
		cerr << "(CMPU6050::CMPU6050) Echo to " << in_accel_x_en
				<< " failed! ret: " << ret << endl;
		exit(-1);
	}
	...
	if (ret < 0) {
		cerr << "(CMPU6050::CMPU6050) Echo to " << in_accel_y_en
				<< " failed! ret: " << ret << endl;
		exit(-1);
	}
	...
	if (ret < 0) {
		cerr << "(CMPU6050::CMPU6050) Echo to " << in_accel_z_en
				<< " failed! ret: " << ret << endl;
		exit(-1);
	}
	...
	if (ret < 0) {
		cerr << "(CMPU6050::CMPU6050) Echo to " << in_anglvel_x_en
				<< " failed! ret: " << ret << endl;
		exit(-1);
	}
	...
	if (ret < 0) {
		cerr << "(CMPU6050::CMPU6050) Echo to " << in_anglvel_y_en
				<< " failed! ret: " << ret << endl;
		exit(-1);
	}
	...
	if (ret < 0) {
		cerr << "(CMPU6050::CMPU6050) Echo to " << in_anglvel_z_en
				<< " failed! ret: " << ret << endl;
		exit(-1);
	}
	...
	if (ret < 0) {
		cerr << "(CMPU6050::CMPU6050) Open " << in_accel_x_raw
				<< " failed! ret: " << ret << endl;
		exit(-1);
	} else {
		mAccelXFd = ret;
	...
	if (ret < 0) {
		cerr << "(CMPU6050::CMPU6050)  Open " << in_accel_y_raw
				<< " failed! ret: " << ret << endl;
		exit(-1);
	} else {
		mAccelYFd = ret;
	...
	if (ret < 0) {
		cerr << "(CMPU6050::CMPU6050)  Open " << in_anglvel_z_raw
				<< " failed! ret: " << ret << endl;
		exit(-1);
	} else {
		mAnglVelZFd = ret;
...
bool CMPU6050::fetchValues(SMPU6050Data& data) {
	...
	if (ret < 0) {
		cerr << "(CMPU6050::fetchValues) Read from AccelX failed! ret: " << ret
				<< endl;
		return false;
	} else {
	...
	if (ret < 0) {
		cerr << "(CMPU6050::fetchValues) Read from the AccelY failed! ret: "
				<< ret << endl;
		return false;
	} else {
	...
	if (ret < 0) {
		cerr << "(CMPU6050::fetchValues) Read from the AnglVelZ failed! ret: "
				<< ret << endl;
		return false;
	} else {
