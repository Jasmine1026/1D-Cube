...
CThread::CThread(IRunnable* runnablePtr, EPriority prio) :
		mThreadID(-1), mRunablePtr(runnablePtr), mPrioBase(prio) {
...
void CThread::start() {
	...
	if (ret != 0) {
		std::cerr << "(CThread::start) pthread_create() failed! ret: " << ret
				<< std::endl;
		exit(-1);
	}
	//Cleanup
